...
void paint_window ( HWND handle ) {
	...
	if ( FAILED ( hr ) || !pRenderTarget ) { return; }
	...
	pRenderTarget -> DrawLine ( D2D1::Point2F ( pivot_x, pivot_y ), D2D1::Point2F ( bob_x, bob_y ), pBrush, 3.0f );

	pBrush -> SetColor ( D2D1::ColorF ( D2D1::ColorF::CornflowerBlue ) );
	pRenderTarget -> FillEllipse ( D2D1::Ellipse ( D2D1::Point2F ( bob_x, bob_y ), g_bob_radius, g_bob_radius ), pBrush );
